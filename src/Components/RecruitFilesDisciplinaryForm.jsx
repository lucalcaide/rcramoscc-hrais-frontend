...
const RecruitFilesDisciplinaryForm = () => {
    const [employee, setEmployee] = useState([]);
    const [searchQuery, setSearchQuery] = useState("");
    const [currentPage, setCurrentPage] = useState(1);
    const [showAll, setShowAll] = useState(false);
    const [totalDisciplinaryForms, setTotalDisciplinaryForms] = useState(0); // State for total disciplinary_form
      ...
      axios.get("http://localhost:3000/auth/employee")
        .then((result) => {
          if (result.data.Status) {
            ...
            // Calculate total disciplinary_form
            const total = sortedEmployees.reduce((acc, curr) => acc + (curr.disciplinary_form ? 1 : 0), 0);
            setTotalDisciplinaryForms(total);
          } else {
    ...
    const handleDelete = (id, emp_no) => {
      ...
      if (confirmDelete) {
        axios.delete(`http://localhost:3000/auth/delete_disciplinary_form/${id}`)
          .then(result => {
            if (result.data.Status) {
              ...
              // Then show toast
              toast.success(`Disciplinary Form for employee number '${emp_no}' deleted successfully!`);
              // Update total disciplinary_form count
              setTotalDisciplinaryForms(prev => prev - 1);
            } else {
        ...
        toast.info(`Deletion of Disciplinary Form for employee ${emp_no} cancelled.`);
      }
    ...
    const handleFileChange = (id, emp_no, event) => {
      ...
      if (file) {
        ...
        axios.post(`http://localhost:3000/auth/upload_disciplinary_form/${id}`, formData, {
          ...
          .then(result => {
            if (result.data.Status) {
              setEmployee(employee.map(e => e.id === id ? { ...e, disciplinary_form: result.data.disciplinary_form } : e));
              toast.success(`Disciplinary Form for employee number '${emp_no}' uploaded successfully!`);
              // Update total disciplinary_form count
              setTotalDisciplinaryForms(prev => prev + 1);
            } else {
              toast.error(result.data.Error);
    ...
    return (
      <div className='px-5 mt-3 employee fontstyle'>
        <div className='d-flex justify-content-center mb-2 print-hide'>
          <span style={{ fontSize: '35px' }}>DISCIPLINARY FORMS</span>
        </div>
        ...
        {/* Display total disciplinary_form */}
        <div className="mt-1">
          <p style={{fontSize:'20px'}}>Total Disciplinary Forms: {totalDisciplinaryForms}</p>
        </div>
                  ...
                  <table>
                    <thead>
                      <tr>
                        ...
                        <th style={{ fontSize: '18px' }}>Disciplinary Form</th>
...
export default RecruitFilesDisciplinaryForm
