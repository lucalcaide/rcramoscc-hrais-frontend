...
const RecruitFilesJobOffer = () => {
    const [employee, setEmployee] = useState([]);
    const [searchQuery, setSearchQuery] = useState("");
    const [currentPage, setCurrentPage] = useState(1);
    const [showAll, setShowAll] = useState(false);
    const [totalJobOffers, setTotalJobOffers] = useState(0); // State for total job offer
      ...
      axios.get("http://localhost:3000/auth/employee")
        .then((result) => {
          if (result.data.Status) {
            ...
            const total = sortedEmployees.reduce((acc, curr) => acc + (curr.job_offer ? 1 : 0), 0);
            setTotalJobOffers(total);
          } else {
    ...
    const handleDelete = (id, emp_no) => {
      ...
      if (confirmDelete) {
        axios.delete(`http://localhost:3000/auth/delete_joboffer/${id}`)
          .then(result => {
            if (result.data.Status) {
              ...
              // Then show toast
              toast.success(`Job Offer for employee number '${emp_no}' deleted successfully!`);
              // Update total job offer count
              setTotalJobOffers(prev => prev - 1);
            } else {
        ...
        toast.info(`Deletion of job offer for employee ${emp_no} cancelled.`);
      }
    ...
    const handleFileChange = (id, emp_no, event) => {
      ...
      if (file) {
        ...
        axios.post(`http://localhost:3000/auth/upload_joboffer/${id}`, formData, {
          ...
          .then(result => {
            if (result.data.Status) {
              setEmployee(employee.map(e => e.id === id ? { ...e, job_offer: result.data.job_offer } : e));
              toast.success(`Job Offer for employee number '${emp_no}' uploaded successfully!`);
              // Update total job offer count
              setTotalJobOffers(prev => prev + 1);
            } else {
              toast.error(result.data.Error);
    ...
    return (
      <div className='px-5 mt-3 employee fontstyle'>
        <div className='d-flex justify-content-center mb-2 print-hide'>
          <span style={{ fontSize: '35px' }}> JOB OFFER LIST</span>
        </div>
        ...
        {/* Display total job offer */}
        <div className="mt-1">
          <p style={{fontSize:'20px'}}>Total Job Offers: {totalJobOffers}</p>
        </div>
                  ...
                  <table>
                    <thead>
                      <tr>
                        ...
                        <th style={{ fontSize: '18px' }}>Job Offer</th>
                    ...
                    <tbody>
                      {currentEmployees.map(e => (
                        <tr key={e.id}>
                          ...
                          <td style={{ fontSize: '20px', fontStyle:'italic'}}>{e.job_offer}</td>
...
export default RecruitFilesJobOffer
