...
const RecruitFilesValidId = () => {
    const [employee, setEmployee] = useState([]);
    const [searchQuery, setSearchQuery] = useState("");
    const [currentPage, setCurrentPage] = useState(1);
    const [showAll, setShowAll] = useState(false);
    const [totalIDs, setTotalIDs] = useState(0); // State for total valid_ids
      ...
      axios.get("http://localhost:3000/auth/employee")
        .then((result) => {
          if (result.data.Status) {
            ...
            // Calculate total valid_ids
            const total = sortedEmployees.reduce((acc, curr) => acc + (curr.valid_id ? 1 : 0), 0);
            setTotalIDs(total);
          } else {
    ...
    const handleDelete = (id, emp_no) => {
      ...
      if (confirmDelete) {
        axios.delete(`http://localhost:3000/auth/delete_valid_id/${id}`)
          .then(result => {
            if (result.data.Status) {
              ...
              // Then show toast
              toast.success(`Valid IDs for employee number '${emp_no}' deleted successfully!`);
              // Update total valid_ids count
              setTotalIDs(prev => prev - 1);
            } else {
        ...
        toast.info(`Deletion of Valid IDs for employee ${emp_no} cancelled.`);
      }
    ...
    const handleFileChange = (id, emp_no, event) => {
      ...
      if (file) {
        ...
        axios.post(`http://localhost:3000/auth/upload_valid_id/${id}`, formData, {
          ...
          .then(result => {
            if (result.data.Status) {
              setEmployee(employee.map(e => e.id === id ? { ...e, valid_id: result.data.valid_id } : e));
              toast.success(`Valid IDs for employee number '${emp_no}' uploaded successfully!`);
              // Update total valid ids count
              setTotalIDs(prev => prev + 1);
            } else {
              toast.error(result.data.Error);
            }
    ...
    return (
      <div className='px-5 mt-3 employee fontstyle'>
        <div className='d-flex justify-content-center mb-2 print-hide'>
          <span style={{ fontSize: '35px' }}> VALID IDs LIST</span>
        </div>
        ...
        {/* Display total valid ids */}
        <div className="mt-1">
          <p style={{fontSize:'20px'}}>Total Valid IDs: {totalIDs}</p>
        </div>
                  ...
                  <table>
                    <thead>
                      <tr>
                        ...
                        <th style={{ fontSize: '18px' }}>Valid IDs</th>
...
export default RecruitFilesValidId
